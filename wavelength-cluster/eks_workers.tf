...
locals {
  userdata = <<-EOT
    #!/bin/bash
    set -o xtrace
    /etc/eks/bootstrap.sh ${module.eks_cluster.cluster_id} --kubelet-extra-args '--node-labels="pixie.io/components=true"'
  EOT
...
resource "aws_security_group_rule" "edge_newrelic_1" {
  type              = "ingress"
  from_port         = 31000
  to_port           = 31003
  protocol          = "tcp"
